...
fn str_malloc {n: nat} (len: int n): [l: agz] (uninit_str(l, n)) =
    $extfcall([l: agz] (uninit_str(l, n)), "malloc", len)
...
fn str_init_c {l: agz}{n: nat}
    ...
    let
        fun loop {l: addr}{n: nat} .<n>.
            (s: !uninit_str(l, n) >> str(l, n), 
             c: char, size: int n): void =
            if size = 0 then let
                prval uninit_str_v_nil () = s.0
                prval () = s.0 := str_v_nil ()
            in () end
    ...
    let
        fun loop {l: addr}{l2: addr}{n, m, r: nat | r == n + m}{c: int}
                 ...
                 in
                     ...
                     prval uninit_str_v_cons(p2l, p2r) = s2.0
